...
public final class FriendDB {
  ...
  private boolean isFriendExist(String id) {
    boolean flag = false;
    SQLiteDatabase db = mDbHelper.getWritableDatabase();
    Cursor cursor = db.query(FeedEntry.TABLE_NAME, null, FeedEntry.COLUMN_NAME_ID + "=?", new String[]{id}, null, null, null);
    if (cursor != null) {
  ...
  public long addFriend(Friend friend) {
    ...
    if (isFriendExist(friend.id)) {
      return db.update(FeedEntry.TABLE_NAME, values, FeedEntry.COLUMN_NAME_ID + "=?", new String[]{friend.id});
    } else {
