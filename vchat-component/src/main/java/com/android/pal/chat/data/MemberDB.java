...
public class MemberDB {
  ...
  public Member getMember(String id) {
    ...
    Cursor cursor = db.rawQuery(FeedEntry.QUERY_SELECT_MEMBER_BY_ID, new String[]{id});
    if (cursor != null) {
      if (cursor.getCount() > 0) {
        if (cursor.moveToFirst()) {
          ...
          member.mobile = cursor.getString(5);
        }
      }
      if (!cursor.isClosed()) {
        cursor.close();
  ...
  public long addMember(Member member, String groupId) {
    ...
    values.put(FeedEntry.COLUMN_NAME_MOBILE, member.mobile);
  ...
  public static class FeedEntry implements BaseColumns {
    ...
    static final String QUERY_SELECT_MEMBER_BY_ID = "select * from " + TABLE_NAME
  ...
  private static final String SQL_CREATE_ENTRIES =
    "CREATE TABLE " + FeedEntry.TABLE_NAME + " (" +
      ...
      FeedEntry.COLUMN_NAME_MOBILE + TEXT_TYPE + COMMA_SEP +
